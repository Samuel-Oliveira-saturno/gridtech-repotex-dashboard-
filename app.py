...
import numpy as np
import matplotlib
matplotlib.use("Agg")  # backend não interativo (necessário em servidor)
...
from scipy.cluster.hierarchy import linkage, dendrogram  # <-- dendrograma
...
def gerar_dendrograma(max_samples=200, method="ward"):
    """
    Gera um dendrograma hierárquico a partir de uma amostra das estações.
    - max_samples: número máximo de estações na amostra.
    - method: 'ward', 'average', 'complete', 'single'.
    Usa X_scaled (latitude/longitude padronizadas).
    """
    X = X_scaled.copy()
    n = X.shape[0]
    if n > max_samples:
...
with gr.Blocks(
    ...
    # Seção de dendrograma
    gr.Markdown("### Hierarchical Clustering – Dendrogram")
    with gr.Row():
        dendro_samples = gr.Slider(
            minimum=50, maximum=400, step=50, value=200,
            label="Max stations in dendrogram sample"
        )
    ...
    btn_dendro = gr.Button("Generate Dendrogram")
    ...
    gr.Markdown(
        ...
        "clipped to Brazil's boundary. A dendrogram (hierarchical clustering) provides an "
        "alternative view of spatial similarity between stations."
    )
    ...
    def update_dendrogram(max_samples, method):
        fig = gerar_dendrograma(max_samples=int(max_samples), method=method)
        return fig

    btn_dendro.click(
        fn=update_dendrogram,
