...
# ─────────────────────────────────────────────────────────────────────────────
# Ajuste o BASE_DIR para o diretório onde você salvará seus dados e o script.
# Por exemplo, se o script estiver em 'C:/Projetos/RepoTex/src/main.py',
# e os dados em 'C:/Projetos/RepoTex/data_lake/', então BASE_DIR seria 'C:/Projetos/RepoTex'
BASE_DIR = Path(__file__).resolve().parent.parent # Assume script está em 'src' dentro de BASE_DIR
# Se o script estiver na raiz do projeto, use: BASE_DIR = Path(__file__).resolve().parent

INPUT_PATH = BASE_DIR / "data_lake/gold/station_geo.parquet"
CLIMATE_PATH = BASE_DIR / "data_lake/gold/station_climate.parquet"
MONTHLY_PATH = BASE_DIR / "data_lake/gold/station_climate_monthly.parquet"
GEOJSON_DIR = BASE_DIR / "docs/geojson_outputs"
LOGO_PATH = str(BASE_DIR / "logoGT.png") # Certifique-se de ter um logoGT.png no diretório base

# Linhas de transmissão (GeoParquet com LineString/MultiLineString em EPSG:4326)
LINHAS_TRANSMISSAO_PATH = BASE_DIR / "Data_Lake_KMZ/gold/geoparquet/linhas_transmissao.parquet"
...
if LINHAS_TRANSMISSAO_PATH.exists():
    try:
        ...
        print(f" ERRO ao carregar LTs: {_e}")
else:
...
# Estado global compartilhado entre callbacks Gradio
last_df_temp = None
last_cells = None
last_centroids = None
last_actual_k = None
...
def gerar_voronoi(n_raw, show_lt: bool = True):
    ...
    lt_msg = (f" Transmission lines : {len(gdf_lt)} features\n"
              if has_lt else " Transmission lines : not available\n")
    summary = (
        "Clustering Summary\n" + "─"*44 + "\n"
        f" Total stations : {n_stations_total}\n"
        f" With climate data : {n_with_climate}\n"
        f" Without climate : {n_without_climate}\n"
        f" Stations/cluster : {n_per_cluster}\n"
        f" Clusters generated : {actual_k}\n"
        f" Voronoi cells : {n_valid}\n"
        + lt_msg + "─"*44 + "\n"
        f" Min cluster size : {sizes.min()} stations\n"
        f" Median cluster size: {sizes.median():.0f} stations\n"
        f" Max cluster size : {sizes.max()} stations\n"
        + "─"*44 + "\n"
        f" GeoJSON saved :\n {geojson_path or 'N/A'}"
    )
    ...
    for c in range(actual_k):
        ...
        st_list = (" | ".join(subset[STATION_COL].dropna().astype(str).tolist())
                   if STATION_COL else
                   " | ".join(f"({r.latitude:.2f},{r.longitude:.2f})"
                              for r in subset.itertuples()))
        geo_rows.append({
            "Cluster ID" : int(c+1),
            "N Stations" : int(len(subset)),
            "Centroid Lat": round(subset["latitude"].mean(), 4),
            "Centroid Lon": round(subset["longitude"].mean(), 4),
            "Stations" : st_list,
        })
    ...
    for c in range(actual_k):
        ...
        subset = df_temp[mask]
        if subset.empty: continue
        row = {"Cluster ID": int(c+1)}
        for col in CLIMATE_COLS:
            if col in subset.columns:
                vals = subset[col].dropna()
                row[CLIMATE_LABELS[col]] = (
                    round(float(vals.mean()), 2) if not vals.empty else None
                )
        clim_rows.append(row)
    clim_df = pd.DataFrame(clim_rows)

    folium_html = gerar_mapa_folium(show_lt=show_lt)
    return fig, summary, geo_df, clim_df, folium_html

# ─────────────────────────────────────────────────────────────────────────────
# 13. CLUSTER DETAIL
# ─────────────────────────────────────────────────────────────────────────────
def cluster_station_details(cluster_id_str: str):
    """Tabela de estações, tensor NumPy e lista de nomes do cluster."""
    global last_df_temp, last_cluster_id
    if last_df_temp is None or not cluster_id_str:
        return pd.DataFrame(), "", "Run clustering first.", []
    last_cluster_id = cluster_id_str
    c_idx = int(cluster_id_str) - 1
    subset = last_df_temp[last_df_temp["cluster_geo"] == c_idx]
    if subset.empty:
        return pd.DataFrame(), "", f"No stations in cluster {cluster_id_str}.", []

    cols = ([STATION_COL,"latitude","longitude"] if STATION_COL
            else ["latitude","longitude"])
    df_out = subset[cols].reset_index(drop=True)
    df_out.columns = [c.replace("_"," ").title() for c in df_out.columns]

    tensor = np.array(subset[["latitude","longitude"]].values, dtype=float)
    choices = (subset[STATION_COL].dropna().astype(str).tolist() if STATION_COL
               else [f"Lat {r.latitude:.3f}|Lon {r.longitude:.3f}"
                     for r in subset.itertuples()])
    return df_out, str(tensor), f"Tensor shape: {tensor.shape}", choices

def climate_summary_for_cluster(cluster_id_str: str) -> pd.DataFrame:
    """Estatísticas climáticas (Mean|Min|Max|N) do cluster."""
    global last_df_temp
    if last_df_temp is None or not cluster_id_str:
        return pd.DataFrame()
    c_idx = int(cluster_id_str) - 1
    ...
    for col in CLIMATE_COLS:
        ...
        rows.append({
            ...
            "Mean" : round(float(vals.mean()), 3) if not vals.empty else "N/A",
            "Min" : round(float(vals.min()), 3) if not vals.empty else "N/A",
            "Max" : round(float(vals.max()), 3) if not vals.empty else "N/A",
            "Stations w/ Data": int(vals.count()),
        })
    return pd.DataFrame(rows)

def plot_cluster_voronoi_only(cluster_id_str: str, show_lt: bool = True):
    """Mapa Matplotlib de detalhe do cluster selecionado."""
    global last_cells, last_actual_k, last_df_temp
    if last_cells is None or not cluster_id_str: return None
    c_idx = int(cluster_id_str) - 1
    if not (0 <= c_idx < last_actual_k): return None
    ...
    for i, cell in enumerate(last_cells):
        ...
        try:
            ...
            for p in polys:
                x, y = p.exterior.xy
                ax.fill(x, y, alpha=0.16, color="#CFD8DC", zorder=2)
                ax.plot(x, y, color="#78909C", lw=0.35, zorder=3)
        except Exception:
            continue

    cell_sel = last_cells[c_idx] if c_idx < len(last_cells) else None
    if cell_sel and not cell_sel.is_empty:
        try:
            polys = [cell_sel] if isinstance(cell_sel, Polygon) else list(cell_sel.geoms)
            for p in polys:
                x, y = p.exterior.xy
                ax.fill(x, y, alpha=0.38, color="#1976D2", zorder=4)
                ax.plot(x, y, color="#0D47A1", lw=0.9, zorder=5)
        except Exception:
            pass

    if last_df_temp is not None:
        sub = last_df_temp[last_df_temp["cluster_geo"] == c_idx]
        if not sub.empty:
            ax.scatter(sub["longitude"], sub["latitude"],
                       s=32, color="#FFFFFF", edgecolor="#0D47A1",
                       linewidth=0.6, alpha=0.95, zorder=6,
                       label="Stations in cluster")
    ...
    if lt_handle:
        handles.append(lt_handle)
        labels_leg.append("Transmission Lines")
    ax.legend(handles, labels_leg, loc="lower right", fontsize=8, framealpha=0.85)

    ax.set_title(
        f"Voronoi Region — Cluster {cluster_id_str}"
        + (" | + LT" if show_lt and has_lt else ""),
        fontsize=12, fontweight="bold", color="#1565C0", pad=10,
    )
...
def gerar_heatmap_clima(climate_var: str, show_lt: bool = True):
    """Mapa de calor com regiões Voronoi coloridas pela média climática do cluster."""
    global last_df_temp, last_cells, last_actual_k
    if last_df_temp is None or last_cells is None: return None
    if climate_var not in last_df_temp.columns: return None
    plt.close("all")
    ...
    cluster_means = {
        c: float(last_df_temp.loc[last_df_temp["cluster_geo"]==c, climate_var]
                 .dropna().mean())
        for c in range(last_actual_k)
    }
    values = [v for v in cluster_means.values() if not np.isnan(v)]
    if not values: return None
    norm = mcolors.Normalize(vmin=min(values), vmax=max(values))

    fig, ax = plt.subplots(figsize=(10, 11))
    fig.patch.set_facecolor("#F7F9FB")
    gdf_br.plot(ax=ax, color="#ECEFF1", edgecolor="#335C67", linewidth=0.8, zorder=1)
    if has_states:
        gdf_states.plot(ax=ax, color="none", edgecolor="#B0BEC5",
    ...
    for i, cell in enumerate(last_cells):
        if cell is None or cell.is_empty: continue
        mv = cluster_means.get(i, np.nan)
        color = "#E0E0E0" if np.isnan(mv) else mcolors.to_hex(cmap_obj(norm(mv)))
        alpha_ = 0.40 if np.isnan(mv) else 0.78
        try:
            polys = [cell] if isinstance(cell, Polygon) else list(cell.geoms)
            for p in polys:
                x, y = p.exterior.xy
                ax.fill(x, y, alpha=alpha_, color=color, zorder=3)
                ax.plot(x, y, color="#78909C", lw=0.35, zorder=4)
        except Exception:
            continue

    ax.scatter(df["longitude"], df["latitude"],
               s=5, alpha=0.5, color="#263238", edgecolors="none", zorder=5)

    lt_handle = plot_linhas_transmissao(ax) if show_lt else None

    sm = cm.ScalarMappable(cmap=cmap_obj, norm=norm)
    sm.set_array([])
    cbar = fig.colorbar(sm, ax=ax, fraction=0.028, pad=0.03, shrink=0.6)
    cbar.set_label(f"{label} ({unit})", fontsize=11, labelpad=10)
    cbar.ax.tick_params(labelsize=9)

    if lt_handle:
        ax.legend(handles=[lt_handle], loc="lower right", fontsize=9, framealpha=0.85)

    ax.set_title(
        f"Climate Heatmap — {label}\nVoronoi Regions Colored by Cluster Mean"
        + (" | + LT" if show_lt and has_lt else ""),
        fontsize=12, fontweight="bold", color="#1565C0", pad=14,
    )
    ax.set_xlabel("Longitude", fontsize=10)
    ax.set_ylabel("Latitude", fontsize=10)
    ax.annotate(
        f"Range: {min(values):.1f} → {max(values):.1f} {unit}",
        xy=(0.02, 0.03), xycoords="axes fraction", fontsize=9, color="#37474F",
...
# ─────────────────────────────────────────────────────────────────────────────
# 15. BOXPLOT HELPERS — compartilhados entre análise agregada e mensal
# ─────────────────────────────────────────────────────────────────────────────
def _style_bp(bp, n: int, cmap_obj, unit: str) -> None:
    """Estilo visual padronizado para boxplots Matplotlib."""
    for idx, patch in enumerate(bp["boxes"]):
        patch.set_facecolor(mcolors.to_hex(cmap_obj(idx / max(n-1, 1))))
    ...
    for wh in bp["whiskers"]:
        wh.set_linestyle("--")
        wh.set_color("#546E7A")
        wh.set_linewidth(0.9)
    for cap in bp["caps"]:
        cap.set_color("#546E7A")
        cap.set_linewidth(1.0)
    for fl in bp["fliers"]:
        fl.set(marker="o", markersize=3, alpha=0.45,
               markerfacecolor="#455A64", linestyle="none")

def _annotate_medians(ax, data_list: list, positions,
                      unit: str, suppress: bool = False) -> None:
    """Anotações de mediana acima de cada caixa (suprimível)."""
    if suppress: return
    for pos, vals in zip(positions, data_list):
        if len(vals) == 0: continue
        med = float(np.median(vals))
        ax.annotate(
            f"{med:.1f}{unit}", xy=(pos, med),
            xytext=(0, 9), textcoords="offset points",
            ha="center", fontsize=7.5, color="#1A237E",
            bbox=dict(boxstyle="round,pad=0.15", fc="white", alpha=0.55),
        )

def _reference_line(ax, data_list: list, unit: str,
                    loc: str = "upper right") -> None:
    """Linha horizontal na média geral de todos os dados."""
    concat = np.concatenate([v for v in data_list if len(v) > 0])
    if len(concat) > 0:
        gm = float(np.mean(concat))
        ax.axhline(gm, color="#E53935", linestyle=":", linewidth=1.2,
                   label=f"Mean: {gm:.1f} {unit}")
        ax.legend(fontsize=8, framealpha=0.8, loc=loc)

def _build_stats_table(sub_df, climate_var: str,
                       group_col: str, label_map: dict, unit: str) -> pd.DataFrame:
    """Tabela de estatísticas descritivas agrupada por group_col."""
    rows = []
    for key in sorted(sub_df[group_col].unique()):
        vals = sub_df.loc[sub_df[group_col]==key, climate_var].dropna().values
        if len(vals) == 0: continue
        rows.append({
            "Period" : label_map.get(key, str(key)),
            "N" : int(len(vals)),
            f"Mean ({unit})" : round(float(np.mean(vals)), 2),
            f"Std ({unit})" : round(float(np.std(vals)), 2),
            f"Min ({unit})" : round(float(np.min(vals)), 2),
            f"Q1 ({unit})" : round(float(np.percentile(vals, 25)), 2),
            f"Median ({unit})": round(float(np.median(vals)),2),
            f"Q3 ({unit})" : round(float(np.percentile(vals, 75)), 2),
            f"Max ({unit})" : round(float(np.max(vals)), 2),
        })
    return pd.DataFrame(rows)

def _get_cluster_station_ids(cluster_id_str: str) -> list:
    """station_ids do cluster selecionado."""
    global last_df_temp
    if last_df_temp is None or not cluster_id_str: return []
    c_idx = int(cluster_id_str) - 1
    subset = last_df_temp[last_df_temp["cluster_geo"] == c_idx]
    if subset.empty or "station_id" not in subset.columns: return []
    return subset["station_id"].dropna().astype(str).tolist()

# ─────────────────────────────────────────────────────────────────────────────
# 16. BOXPLOT AGREGADO (sub-modo "Aggregate")
# ─────────────────────────────────────────────────────────────────────────────
def _build_boxplot_base(data_list: list, tick_labels: list,
                        climate_var: str, title: str,
                        compact: bool = False):
    """
    Renderiza múltiplos boxplots lado a lado num painel único.
    compact=True: caixas mais estreitas e separadores verticais —
    usado pelo modo "All Stations Side-by-Side".
    """
    plt.close("all")
    label = CLIMATE_LABELS.get(climate_var, climate_var)
    unit = CLIMATE_UNITS.get(climate_var, "")
...
def _resolve_vals(row_df, climate_var: str, station_id: str = None) -> np.ndarray:
    ...
    if not vals and climate_var in row_df.columns:
...
def gerar_boxplot_agregado(cluster_id_str: str, climate_var: str,
    ...
    if mode == "single":
        ...
        row = (subset[subset[STATION_COL].astype(str)==sname]
               if STATION_COL and STATION_COL in subset.columns
               else subset)
        sid = str(row["station_id"].values[0]) if "station_id" in row.columns and len(row) else None
        vals = _resolve_vals(row, climate_var, sid)
        if not len(vals): return None
        return _build_boxplot_base(
...
def gerar_boxplot_mensal(cluster_id_str: str, climate_var: str,
    ...
    """
    global df_monthly, last_df_temp
...
# 22. LAUNCH
# ─────────────────────────────────────────────────────────────────────────────
app.launch(share=False, debug=True) # Alterado para debug=True para melhor depuração local